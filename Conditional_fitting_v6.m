 %% APO BASAL Activity Running script

f = msgbox("Did you check for STED PSF and zThresh??","REMINDER","error");
STED_PSF=0;
...
%% Loading of data
% Load APO data SNAP
[nom, ruta] = uigetfile('*.mat','Select One or More Files', ...
    'MultiSelect', 'on');
for p=1:length(nom)
    data{1}(1,p) = open(fullfile(ruta,nom{p}));
    APO.B1AR(p).results.zValueAvg=data{1}(1,p).results.zValueAvg;
    APO.B1AR(p).results.zErrorAvg=data{1}(1,p).results.zErrorAvg;
    APO.B1AR(p).results.hAvg=data{1}(1,p).results.hAvg;
    APO.B1AR(p).results.kAvg=data{1}(1,p).results.kAvg;
    APO.B1AR(p).results.hErrorAvg=data{1}(1,p).results.hErrorAvg;
    APO.B1AR(p).results.k1=data{1}(1,p).results.k1;
    APO.B1AR(p).results.k2=data{1}(1,p).results.k2;
    
    APO.B1AR(p).outputs.sig=data{1}(1,p).outputs.sig;
    if nanmedian(APO.B1AR(p).outputs.sig(:))<1
        APO.B1AR(p).outputs.sig=(1.655/(2*sqrt(2*log(2))))./sqrt( APO.B1AR(p).outputs.sig);
    else
        APO.B1AR(p).outputs.sig=APO.B1AR(p).outputs.sig;
    end 
    APO.B1AR(p).outputs.zPos=data{1}(1,p).outputs.zPos;
    APO.B1AR(p).outputs.maxInt=data{1}(1,p).outputs.maxInt;
    APO.B1AR(p).normMaxInt=data{1}(1,p).normMaxInt;
    APO.B1AR(p).normMaxIntStd=data{1}(1,p).normMaxIntStd;
    clear data
end
...
for p=1:length(nom)
    data{1}(1,p) = open(fullfile(ruta,nom{p}));
    APO.GFP(p).avgImageData=data{1}(1,p).avgImageData;
    clear data
end
...
%         constant_tail(i)=max(std_norm_trace(round(zValue(i)+sigma(i)):end));
%         constant_tail2(i)=max(std_norm_trace(round(85:end)));
        z_thresh=6;
...
APO.Fit(p).fitTypeAll=sum(fitType,2);
    ...
    figure,
    bar(1,100*sum(APO.Fit(p).fitTypeAll==1)./(length(APO.Fit(p).fitTypeAll)-sum(isnan(APO.Fit(p).fitTypeAll))))
    hold on
    bar(2,100*sum(APO.Fit(p).fitTypeAll==2)./(length(APO.Fit(p).fitTypeAll)-sum(isnan(APO.Fit(p).fitTypeAll))))
    hold on
    bar(3,100*sum(APO.Fit(p).fitTypeAll==3)./(length(APO.Fit(p).fitTypeAll)-sum(isnan(APO.Fit(p).fitTypeAll))))
    hold on
    bar(4,100*sum(APO.Fit(p).fitTypeAll==4)./(length(APO.Fit(p).fitTypeAll)-sum(isnan(APO.Fit(p).fitTypeAll))))
    hold on
    bar(5,100*sum(APO.Fit(p).fitTypeAll==5)./(length(APO.Fit(p).fitTypeAll)-sum(isnan(APO.Fit(p).fitTypeAll))))
    xticks([1 2 3 4 5])
    xlabel('FitType')
    ylabel('Occurence [%]')
    set(gca,'FontSize',24,'linewidth',2,'TickDir','out','Ydir','normal')
    set(gcf,'color','w'),
...
% % %             sigmoidF=aCoef(i)./(1+exp(-(-0.0002278*bCoef(i).^3+0.008699*bCoef(i).^2-0.1226*bCoef(i)+0.7494).*(zData-cCoef(i)))) + kCoef(i);
% % %             FlatTopGaussF=eCoef(i).*exp(-(((zData-fCoef(i)).^2)./(2.*bCoef(i)).^2).^gCoef(i))+ kCoef(i);
% % %             LorF=dCoef(i).*exp(-(1.656/(2.355*bCoef(i))).^2*0.25*(zData-cCoef(i)).^2)./(1+((1.656/(2.355*bCoef(i))).^2)*((zData-cCoef(i)).^2))+ kCoef(i);
     
%             sigmoidF=aCoef(i)./(1+exp(-(-0.0002278*bCoef(i).^3+0.008699*bCoef(i).^2-0.1226.*bCoef(i)+0.7494).*(zData-cCoef(i)))) + ones(length(zData),1).*kCoef(i);
%             
%             LorF=dCoef(i).*exp(-(1.656./(2.355.*bCoef(i))).^2.*0.25*(zData-cCoef(i)).^2)./(1+(1.656./(2.355*bCoef(i))).^2*((zData-cCoef(i)).^2))+ ones(length(zData),1).*kCoef(i);
%             
%             FlatTopGaussF=eCoef(i).*exp(-(((zData-fCoef(i)).^2)./(2.*(bCoef(i)).^2).^gCoef(i)))+ ones(length(zData),1).*kCoef(i);
% 
%             
%             
%             figure%('Renderer', 'painters', 'Position', [600 600 900 600])
%             plot(f,zData,e);
%             hold on
%             plot(zData,sigmoidF,'LineWidth',2)
%             hold on
%             plot(zData,LorF,'LineWidth',2)
%             hold on
%             plot(zData,FlatTopGaussF,'LineWidth',2)
%             hold on
%             plot(zData, sigmoidF+LorF+FlatTopGaussF-2*ones(length(zData),1).*kCoef(i))
%             hold on
%             plot([zValue(i) zValue(i)], [0, 20] )
%             title(['FitType ', num2str(fitType(i)),' pos ',num2str(i),' rsq ',num2str(gof.adjrsquare)])
...
%     zValMask=~isnan(APO.B1AR(p).results.zValueAvg);  
%     mask=filteredRes>0;
%     
%     for i=1:size(mask,1)
%     for j=1:size(mask,2)
%         
%         if mask(i,j)==0 && zValMask(i,j)==0
%         filteredRes(i,j)=0.1;
%         else
%             continue
%         end 
